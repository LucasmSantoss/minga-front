...
export default function DetailButtons({ info }) {
  ...
  useEffect(() => { dispatch(captureChapter({ manga_id: info._id, page: pagination })); }, [pagination, capitulo]);
  return (
    <>
...
<div className='div-chapter'>
  ...
  {chapters?.length > 0 && <Anchor className='btn-chapter' to={'/manga/' + info._id + '/' + (pagination + 1)}><button className='btn-chapter' onClick={() => setPagination(pagination + 1)}>next</button></Anchor>}
</div>
